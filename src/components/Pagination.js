...
export default function Pagination({ currentPage, numPages, categoryName }) {
  ...
  const prevPage = categoryName
    ? `/category/${categoryName}/page/${currentPage - 1}`
    : `/page/${currentPage - 1}`;
  const nextPage = categoryName
    ? `/category/${categoryName}/page/${currentPage + 1}`
    : `/page/${currentPage + 1}`;
      ...
      <ul className="pagination-inner">
        {!isFirst && (
          <Link href={prevPage} passHref>
            <li className="pagination-button">Previous</li>
          </Link>
        )}
        ...
        {!isLast && (
          <Link href={nextPage} passHref>
            <li className="pagination-button">Next</li>
          </Link>
        )}
      </ul>
    </div>
